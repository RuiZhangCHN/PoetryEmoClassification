...
# ========== Albert Base ==========
from transformers import AlbertTokenizerFast, AlbertForSequenceClassification, AlbertConfig
tokenizer = AlbertTokenizerFast.from_pretrained('voidful/albert_chinese_base')
...
device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
configuration = AlbertConfig.from_pretrained('voidful/albert_chinese_base', num_labels=5)
model = AlbertForSequenceClassification.from_pretrained('voidful/albert_chinese_base', config=configuration)
model.to(device)
